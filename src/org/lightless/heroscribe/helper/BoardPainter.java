...
public class BoardPainter implements ImageObserver {
  ...
  public void init() {
    boardSize = new Dimension(gui.getObjects().getBoard().width, gui
        .getObjects().getBoard().height);
    ...
    adjacentBoardsOffset = Math.round(boxEdge
        * gui.getObjects().getBoard().adjacentBoardsOffset);

    framePixelSize = new Dimension(boardPixelSize.width
        * gui.getQuest().getWidth() + adjacentBoardsOffset
        * (gui.getQuest().getWidth() - 1), boardPixelSize.height
        * gui.getQuest().getHeight() + adjacentBoardsOffset
        * (gui.getQuest().getHeight() - 1));
  }

  private void drawBridge(int column, int row, boolean horizontal,
      int position, Graphics2D g2d) {
    int x, y;
    int width, height;
  ...
  private void drawRectangle(int column, int row, float left, float top,
      float width, float height, Graphics2D g2d) {

    g2d.fillRect(getIntX(column, left), getIntY(row, top),
        (int) Math.ceil(width * boxEdge), (int) Math.ceil(height * boxEdge));
  }

  private float getX(int column, float left) {
    return column * (boardPixelSize.width + adjacentBoardsOffset) + left
        * boxEdge;
  }
