...
public class ExportEPS {
  ...
  private static int[] appendPS(String inPath, PrintWriter out,
      boolean printComments, boolean divideInBlocks) throws Exception {
  ...
  public static void write(File file, Quest quest, List objects)
    ...
    try (PrintWriter out = new PrintWriter(new BufferedWriter(new FileWriter(file)))) {
      ...
      bBoxWidth = (quest.getWidth() * (quest.getBoard(0, 0).getWidth() + 2) + (quest
          .getWidth() - 1) * objects.getBoard().adjacentBoardsOffset) * 19.2f;

      bBoxHeight = (quest.getHeight() * (quest.getBoard(0, 0).getHeight() + 2) + (quest
          .getHeight() - 1) * objects.getBoard().adjacentBoardsOffset) * 19.2f;

      out.println("%!PS-Adobe-3.0 EPSF-3.0");
      out.println("%%LanguageLevel: 2");
      out.println("%%BoundingBox: 0 0 " + Math.round(Math.ceil(bBoxWidth)) + " "
      ...
      for (String id : set) {
        ...
        /*
         * the postscript is divided in "{ } exec" blocks to broaden compatibility
         */
        boundingBox = appendPS(objects.getVectorPath(id, quest.getRegion()), out,
            false, true);

        out.println(" } bind /Matrix [1 0 0 1 0 0] /BBox [" + boundingBox[0]
        ...
        for (int row = 0; row < quest.getHeight(); row++) {
          ...
          for (QObject obj : board.objectsIterable()) {
            ...
            if (objects.getObject(obj.id).trap) {
              out.println(obj.left + " "
                  + (board.getHeight() - obj.top - height + 2) + " " + width
                  + " " + height + " Trap");
            ...
            default:
              throw new VerifyError("Bad rotation!");
            }
